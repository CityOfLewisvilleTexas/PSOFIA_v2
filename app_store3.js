        ...
        <v-row no-gutters>
            <v-col cols="12" sm="12">
                <v-card :loading="isLoading">
                    <v-toolbar flat v-if="storeHasData">
                        <psofia-webservice-settings :store-name="storeName" :form-id="formID"
                            v-if="!isLoading && storeName=='recordsList'"
...
// LIST
    computed: {
        ...
        storeHasData: function(){
            return store.getStoreHasData(this.routePayload);
        },
        allForms: function(){
        ...
        allItems: function(){
            ...
            else return (this.storeName == 'formsList') ? this.forms : this.records;
        },
        ...
        countTotal: function(){ return (this._allItems) ? this._allItems.length : 0; },
        ...
        countInactive: function(){
            if(this._allItems && this._allItems.length){
                return this._allItems.filter(function(item){ return !(item.Active.val); }).length;
            }
            else return 0;
        },
        ...
        tableHeaders: function(){
            if(this.storeHasData) if(this._allItems) return store.getColumns_Headers(this.storeName);  
        },
        ...
        wsProps: function(){ if(this.storeTableSettings) return this.storeTableSettings.wsProps; },
        ...
        pageTitle: function(){
            ...
            if(!this.isLoading){
                if(this.storeName == 'recordsList'){
                    if(this.formData) title = this.formData.FormName.displayVal;
        ...
        getRecords: function(callType){     //callTypes - 0: initial, 1: change ws settings, 2: after delete/restore
            ...
            if(this.hasInternet){
                if(this.params.formID){
                    store.setStoreIsLoading({isLoading: true});
                    store.setWSProps({storeName: this.storeName, formID: this.params.formID});
                    ...
                    function(data){
                        ...
                        if(data.Columns[0].ID == -1){
                            ...
                            if(callType == 0){
                                ...
                                if(self.formID && self.params.formID != self.formID){
                                    //don't show other form records
                                }
                                else if(self.formID && self.params.formID == self.formID){
                                    //notify most recent get wasn't complete
                                    if(callType == 1){
                            ...
                            //if(data.FormData && data.FormData.length == 1){
                                store.setReturnFormID(data.FormData[0].FormID);
                    ...
                    .fail(function(data){
                        ...
                        Vue.nextTick(function(){
                            if(self.totalRecords > 0) store.setStoreIsLoading({isLoading: false});
